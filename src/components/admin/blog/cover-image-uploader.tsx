...
import { uploadBlogCoverImage, ACCEPTED_BLOG_IMAGE_TYPES, MAX_BLOG_IMAGE_SIZE_BYTES } from "@/lib/blog-upload";
...
export function CoverImageUploader({ value, onChange, draftId }: CoverImageUploaderProps) {
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setIsUploading(true);
    try {
      const url = await uploadBlogCoverImage(file, draftId);
      onChange(url);
    } catch (error) {
